...
namespace SmartAccess
{
    ...
    public static class WebClient
    {
        ...
        /// <param name="ms_timeout">Timeout in milliseconds</param>

        /// <returns>An HttpRequest object on success, null otherwise</returns>
        public static HttpRequest GetFromWeb(string url, int ms_timeout)
        {
            HttpRequest request = HttpHelper.CreateHttpGetRequest(url,ms_timeout);
            if (request != null)
            {
                if (Proxy != null)
                {
