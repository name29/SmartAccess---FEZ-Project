...
namespace SmartAccess
{
    ...
    public class HttpRequest
    {
        ...
        private int ms_timeout = 2000;
        private static string[] RequestMethodString = { "GET", "PUT", "POST", "DELETE" };
        ...
        /// </summary>
        public RequestMethod HttpRequestMethod { get { return httpRequestMethod; } }
        ...
        /// </summary>
        public bool IsReceived { get { return isReceived; } }
        private bool isReceived;

        /// <summary>
        /// Specifies if an Exception was Thrown
        /// </summary>
        public bool IsExceptionThrown { get { return isExceptionThrown; } }
        ...
        /// <summary>
        /// The exception thrown during the HTTP request
        /// </summary>
        ...
        /// <summary>
        /// Specifies the signature for the method that is called if a generic Exception is thrown during the HTTP Request/Response process. 
        /// </summary>
        /// <param name="sender">The <see cref="HttpRequest"/> object that raised the event</param>
        /// <param name="e">The <see cref="Exception"/> object of the Exception (Try cast to WebException for more details)</param>
        public delegate void ExceptionHandler(HttpRequest sender, Exception e);
        ...
        ///<summary>
        /// Event used for an asynchronously handled exception.
        ///</summary>
        public event ExceptionHandler ExceptionThrown
        {
            add
            {
                //lock (_Lock)
                {
                    ...
                    // invoke newly added event handler directly if an exception was already thrown 
                    if (isExceptionThrown)
                    {
                        Program.BeginInvoke(value, this, exceptionThrown);
                    }
                }
            }
            remove
        ...
        /// The internally used event that includes the added handlers.
        /// </summary>
        private ExceptionHandler _ExceptionThrown;
        ...
        /// The internally used event handler for the event pattern specified by the rest of the Gadgeteer API.
        /// </summary>
        private ExceptionHandler _OnExReceived;
        ...
        /// <summary>
        /// Raises the <see cref="HttpRequest.ExceptionThrown"/> event.
        /// </summary>
        /// <param name="sender">The <see cref="HttpRequest"/> object that raised the event</param>
        /// <param name="e">The <see cref="Exception"/> object of the exception</param>
        internal void OnExceptionThrownEvent(HttpRequest sender, Exception e)
        {
            if (_OnExReceived == null)
            {
                _OnExReceived = new ExceptionHandler(OnExceptionThrownEvent);
            }
            if (Program.CheckAndInvoke(_ExceptionThrown, _OnExReceived, sender, exceptionThrown))
            {
                _ExceptionThrown(this, exceptionThrown);
            }
        }
        ...
        private void HandleRequestSync()
        {
            try
            {
                lock (_Lock)
                {
                    if (isExceptionThrown)
                    {
                        OnExceptionThrownEvent(this, exceptionThrown);
                        return;
                    }
        ...
        public void SendRequest()
        {
            ...
            {
                throw new Exception("You must define an ExceptionHandler before call sendRequest");
            }

            if (_ResponseReceived == null )
            {
                throw new Exception("You must define a ReceiverHandler before call sendRequest");
            }
        ...
        /// <param name="ms_timeout">Timeout for the HTTP request</param>
        internal HttpRequest(RequestMethod method, string URL, HttpContent content, string contentType, string acceptsType, string userAgent, int ms_timeout)
        {
            //always necessary 
