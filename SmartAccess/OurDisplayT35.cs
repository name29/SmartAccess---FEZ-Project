...
namespace SmartAccess
{
    ...
    /// </summary>
    /// <example>
    /// <para>The following example uses a <see cref="OurDisplayT35"/> object to display the picture taken by a camera module. 
    ...
    /// </code>
    /// </example>
    public class OurDisplayT35 : GTM.Module.DisplayModule
    {
        ...
        public OurDisplayT35(int rgbSocketNumber1, int rgbSocketNumber2, int rgbSocketNumber3)
            : this(rgbSocketNumber1, rgbSocketNumber2, rgbSocketNumber3, GT.Socket.Unused)
        {
        }
        ...
        public OurDisplayT35(int rgbSocketNumber1, int rgbSocketNumber2, int rgbSocketNumber3, int touchSocketNumber)
            : base(WpfMode.PassThrough)
        {
            ReserveLCDPins(rgbSocketNumber1, rgbSocketNumber2, rgbSocketNumber3);
            ConfigureLCD();

            if (touchSocketNumber == GT.Socket.Unused) return;
        ...
        private static GT.Socket greenSocket;
        private void ReserveLCDPins(int rgbSocketNumber1, int rgbSocketNumber2, int rgbSocketNumber3)
        {
            ...
            GT.Socket[] rgbSockets = new GT.Socket[3] { GT.Socket.GetSocket(rgbSocketNumber1, true, this, "rgbSocket1"), GT.Socket.GetSocket(rgbSocketNumber2, true, this, "rgbSocket2"), GT.Socket.GetSocket(rgbSocketNumber3, true, this, "rgbSocket3") };
            ...
            foreach (var rgbSocket in rgbSockets)
            {
                ...
                else if (!gotG && rgbSocket.SupportsType('G'))
                {
                    ...
                    backlightPin = GTI.DigitalOutputFactory.Create(greenSocket, GT.Socket.Pin.Nine, true, this);
                }
                ...
                rgbSocket.ReservePin(GT.Socket.Pin.Three, this);
                rgbSocket.ReservePin(GT.Socket.Pin.Four, this);
                rgbSocket.ReservePin(GT.Socket.Pin.Five, this);
                rgbSocket.ReservePin(GT.Socket.Pin.Six, this);
                rgbSocket.ReservePin(GT.Socket.Pin.Seven, this);
                rgbSocket.ReservePin(GT.Socket.Pin.Eight, this);

                if (!rgbSocket.SupportsType('G'))
                    rgbSocket.ReservePin(GT.Socket.Pin.Nine, this);
            }
        }

        private void ReserveTouchPins(int touchSocketNumber)
        {
            GT.Socket tsocket = GT.Socket.GetSocket(touchSocketNumber, true, this, "T");

            tsocket.EnsureTypeIsSupported('T', this);
            tsocket.ReservePin(GT.Socket.Pin.Four, this);
            tsocket.ReservePin(GT.Socket.Pin.Five, this);
            tsocket.ReservePin(GT.Socket.Pin.Six, this);
            tsocket.ReservePin(GT.Socket.Pin.Seven, this);
        }
